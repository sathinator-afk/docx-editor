...
function insertCommentRanges(content: ParagraphContent[], paragraph: PMNode): ParagraphContent[] {
  ...
  paragraph.forEach((node) => {
    ...
    for (const cid of endsAt.get(nodeIndex) ?? []) {
      ...
      // Emit the comment's reference run (its ECMA-376 anchor). The serializer
      // only writes <w:commentReference> from this 'commentReference' node, and
      // editor comments are marks, so without it every editor-saved comment
      // loses its reference. See eigenpal/docx-editor#837.
